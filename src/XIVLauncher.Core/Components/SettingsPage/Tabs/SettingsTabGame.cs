...
public class SettingsTabGame : SettingsTab
{
    public override SettingsEntry[] Entries { get; } =
    {
        ...
        new SettingsEntry<bool>("Ignore Steam", "Check this if you do not want XIVLauncher to communicate with Steam (Requires Restart). Not Recommended for Steam Deck users.", () => Program.Config.IsIgnoringSteam ?? false, x => Program.Config.IsIgnoringSteam = x)
        {
            CheckWarning = x => (x && Program.IsSteamDeckHardware) ? "Warning! This option can cause Steam Deck input to fail or work incorrectly." : null
        },
    };
